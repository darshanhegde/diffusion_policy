...
class QPLayer(nn.Module):
    ...
    def forward(self, actions, run_inference=False):
        ...
        for i in range(batch_size):
            ...
            x = cp.Variable(output_dims)
            objective = cp.Minimize(0.5 * cp.quad_form(x, Q.cpu().numpy()) + x_prev[i].cpu().numpy().T @ x)
            ...
            x_value = x.value
            assert np.all(G.cpu().numpy() @ x_value <= h.cpu().numpy() + self.eps), "Constraint not satisfied"
            if run_inference:
                assert np.linalg.norm(x_value - center_vector, 2) >= radius + self.eps, "Circular constraint not satisfied"

            # Get the result and convert it to a torch tensor
            x_result = torch.tensor(x.value).to("cuda")
            actions_list.append(x_result)
        
        actions = torch.stack(actions_list, dim=0)
        actions = actions.view(batch_size, num_steps, self.ac_dim)
